...
setenv_file="/opt/atlassian/confluence/bin/setenv.sh"
echo -en "Set JVM_MINIMUM_MEMORY ... "
sed -i "s/JVM_MINIMUM_MEMORY:=[0-9]*m/JVM_MINIMUM_MEMORY:=${jvm_mem_min}m/" ${setenv_file}
echo -e "Done"
echo -en "Set JVM_MAXIMUM_MEMORY ... "
sed -i "s/JVM_MAXIMUM_MEMORY:=[0-9]*m/JVM_MAXIMUM_MEMORY:=${jvm_mem_max}m/" ${setenv_file}
echo -e "Done"
